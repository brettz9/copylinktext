...
2. API updates
    ...
    2. Propose or implement JSON object type preference accessible/settable without conversion to string
    3. Propose or implement (recover old feature): Allow package.json preferences to indicate dependencies (e.g., if one is enabled, it will disable another pref.); though one can workaround by using radio buttons, sometimes less elegant
    4. Propose or implement: Prompts to SDK API (to avoid need for chrome)? (Also Door-hanger Notifications for asyouwish)
*/
...
function setByPref (pref) {
    ...
    case 'accessFormControls': case 'accessPass': case 'immediateFormClickExecution':
        ...
        break;
    case 'showAltText': case 'copyLinkTextAccessKey': case 'highlightBackgroundColor':
